...
4. **In `dtctl/dashboards/service-health.yaml` and `dtctl/slos/checkout-availability.yaml`** — wraps every reference in `coalesce(customerTier, customer.tier)` so the dashboard and SLO continue to measure during the overlap.
...
| Forgets the `coalesce()` in dashboard/SLO queries | "The overlap window means the SLO needs to see both names. Use coalesce." |
...
3. `dtctl/dashboards/service-health.yaml` AND `dtctl/slos/checkout-availability.yaml`
...
Commit message:
  ...
  - dtctl: dashboard + SLO use coalesce(customerTier, customer.tier)
